...
import { dataInterfaceInterface } from './dev/dataInterface';

const getData = async (): Promise<dataInterfaceInterface> => {
	const response = await fetch(process.env.PG_DATA_ENDPOINT as RequestInfo );
	return await response.json() as dataInterfaceInterface;
};
